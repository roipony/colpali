...
from colpali_engine.models import ColGraniteVision, ColGraniteVisionProcessor
...
def index(file, ds):
    ...
    for f in file:
        ...
        if file_extension == 'pdf':
            ...
            images.extend(convert_from_path(file_path))
        ...
        # Handle image files (jpg, jpeg, png)
            ...
            # For image files, add directly to images list
            from PIL import Image
            img = Image.open(file_path)
            images.append(img)
            
            # Optionally convert image to PDF page and add to pdfWriter
            ...
            # Scale to fit on a letter page
            page_width, page_height = letter
            ratio = min(page_width/img_width, page_height/img_height)
            c.drawImage(file_path, 0, 0, width=img_width*ratio, height=img_height*ratio)
            c.save()
            
            # Add the image page to the PDF writer
            img_buffer.seek(0)
            img_pdf = PdfReader(img_buffer)
            pdfWriter.add_page(img_pdf.pages[0])
...
model_name = "/dccstor/ocr-ai/pony/projects/colpali/models/granite-vision-3.1-2b-preview"
model = cast(
    ColGraniteVision,
    ColGraniteVision.from_pretrained(
        pretrained_model_name_or_path=model_name,
        config= "/dccstor/ocr-ai/pony/projects/colpali/models/granite-vision-3.1-2b-preview/config_base_image_last.json",
        torch_dtype=torch.bfloat16,
        device_map=device,
    ).eval(),
)
model =PeftModel.from_pretrained(model, '/dccstor/ocr-ai/pony/projects/colpali/models/colgranitevision-ckpt-5e-nopad-causal-global-view-last-fixed/').eval()
processor = cast(ColGraniteVisionProcessor, ColGraniteVisionProcessor.from_pretrained(model_name))
...
with gr.Blocks() as demo:
    ...
    gr.Markdown("## 1️⃣ Upload PDFs or Images")
    file = gr.File(file_types=["pdf", "jpg", "jpeg", "png"], file_count="multiple")
